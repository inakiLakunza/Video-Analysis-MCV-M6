...
def transform_window(window, strategy="Central"):
    
    bs, n_ch, n_frames, h, w = window.shape
    no_batch_window = window.squeeze(0)

    if strategy == 'Central':
        transforms = v2.Compose([
            v2.CenterCrop((182, 182)),
            v2.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.0224, 0.225])
        ])
    elif strategy == 'RandomResize':
        transforms = v2.Compose([
            v2.RandomResizedCrop((182, 182)),
        ])
    elif strategy == 'FiveCrop':
        transforms = v2.Compose([
            v2.FiveCrop((182, 182))
        ])
    elif strategy == 'TenCrop':
        transforms = v2.Compose([
            v2.TenCrop((182, 182))
        ])

    # AL HACER FIVE CROP OUT:
    # DEVUELVE UNA LISTA DE LEN N_FRAMES, CON TUPLAS (TENSOR, DEVICE)
    new_window = []
    for clip in no_batch_window.permute(1, 0, 2, 3):
        crops = transforms(clip)
        if isinstance(crops, tuple):
            for crop in crops:
                new_window.append(crop)
        else:
            new_window.append(crops)
    
    new_window = torch.stack(new_window).permute(1, 0, 2, 3).unsqueeze(0)

    return new_window



def evaluate_with_windows(clip, window_size=4, overlap=0, strategy="Central"):
    ...
    for i in range(0, n_frames, jump):
        ...
        generated_windows = transform_window(window, strategy=strategy)
    ...
    # We return a list of a tensor inside
...
if __name__ == "__main__":
    ...
    # LOAD MODEL
    load_path = "/ghome/group07/test/W5/task_2/weights/best_val_model_epoch_83.pth"
    # model.load_state_dict(torch.load(load_path)['model_state_dict']) 
    model.load_state_dict(torch.load(load_path))

    strats = ["Central", "RandomResize", "FiveCrop", "TenCrop"]

    for strategy in strats:
        # Testing
        print(f"======================== {strategy} =======================")
        evaluate(model, loaders['testing'], loss_fn, args.device, None, description=f"Testing", strategy=strategy)
