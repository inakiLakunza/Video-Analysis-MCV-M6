...
def train(
    ...
    for batch in pbar:
        ...
        shape = clips.size()
        dimension_to_shuffle = 2  
  
        
        # for loop en clips
    
        # Reshape the tensor to 2D tensor so that we can shuffle along the chosen dimension
        reshaped_tensor = clips.view(-1, shape[dimension_to_shuffle])

        # Convert tensor to numpy array for shuffling
        reshaped_array = reshaped_tensor.cpu().numpy()

        # Shuffle along the chosen dimension
        np.random.shuffle(reshaped_array)

        # Convert numpy array back to tensor
        shuffled_tensor = torch.from_numpy(reshaped_array)

        # Reshape back to the original shape
        clips_shuffle = (shuffled_tensor.view(shape)).to(device)
        # Forward pass
        outputs = model(clips_shuffle)
        # Compute loss
...
def evaluate(
    ...
    if epoch and (float(hits) / count) >= early_stopper.min_validation_loss:
        # save model
        torch.save(model.state_dict(), f'./weights/best_val_model_epoch_{epoch}.pth')
...
if __name__ == "__main__":
    ...
    wandb.init(
        project = "C6-W6",
        name = "train_task_b epochs=500 early=15",
        config={
    ...
    for epoch in range(args.epochs):
        ...
        if epoch % args.validate_every == 0:
            ...
            val_loss = evaluate(model, loaders['validation'], loss_fn, args.device, epoch, description=description)
