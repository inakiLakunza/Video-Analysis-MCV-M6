...
def display_tsne_plot(features_x, features_y, captions, try_name, title="TSNE"):

    features_data = np.hstack([features_x, features_y[:, None]])
    #features_data = np.hstack([features_x, features_y])
    ...
    plt.tight_layout()
    os.makedirs("./"+try_name, exist_ok=True)
    plt.savefig(f'./{try_name}/{title.replace(" ", "_")}.png', dpi=300)
