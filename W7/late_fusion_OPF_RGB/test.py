...
def min_max_normalize(logits):
    min_logit = logits.min(dim=1, keepdim=True)[0]
    max_logit = logits.max(dim=1, keepdim=True)[0]
    normalized_logits = (logits - min_logit) / (max_logit - min_logit)
...
def evaluate(
    ...
    for batch in pbar:
        ...
        # Forward pass
        with torch.no_grad():
            outputs_rgb = model_rgb(clips_og) * weights_rgb
            outputs_opf = model_opf(clips) * weights_opf

            # Normalize Logits
            # outputs_rgb = min_max_normalize(outputs_rgb) * 0.8
            # outputs_opf = min_max_normalize(outputs_opf) * 0.2

            # Logit Aggregation
            outputs = outputs_rgb + outputs_opf
...
if __name__ == "__main__":
    ...
    model_rgb.load_state_dict(torch.load('./weights/best_val_model_epoch_38_freeze.pth', map_location=args.device))
    ...
    for weight_rgb in weights:
        for weight_opf in weights:
            if np.isclose(weight_rgb + weight_opf, 1.0):
                print(f"Weight combination - RGB: {weight_rgb}, OPF: {weight_opf}")
                evaluate(model_rgb, model_opf, weight_rgb, weight_opf, raft_model, loaders['testing'], loss_fn, args.device, None, description=f"Testing")
