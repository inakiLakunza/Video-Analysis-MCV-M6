        ...
        for line in file:
            ...
            if len(elements) >= 6:
                ...
                box_attributes = {
                    "xtl": int(xtl),
                    "ytl": int(ytl),
                    "xbr": int(float(xtl) + float(w)),
                    "ybr": int(float(ytl) + float(h)),
                }
...
if __name__ == '__main__':
    gt_path = '/ghome/group07/test/W3/task_2/train/S04/c027/gt/gt.txt'
    gt = read_annotations(gt_path)
    images_path = '/ghome/group07/test/W3/task_2/frame_dataset/S04/c027/color/'
    _, color_frames = read_video(images_path, gt)
    print(color_frames.shape)
    #color_frames[170:211]
    os.makedirs('./gt_vid/', exist_ok=True)
    make_video(color_frames[170:211])
