...
from test import evaluate as test_eval
...
def evaluate(
    ...
    if epoch and (float(hits) / count) >= early_stopper.min_validation_loss:
        # save model
        torch.save(model.state_dict(), f'./weights/x3ds_valacc_pretrained.pth')
...
if __name__ == "__main__":
    ...
    model = model_creator.create('x3d_xs', args.load_pretrain, datasets["training"].get_num_classes())
    ...
    SAVE_NAME = "x3ds_valloss_pretrained.pth"
    ...
    weights_to_try = [
        '/ghome/group07/test/W6/task_1/x3d/weights/x3ds_valacc_pretrained.pth',
        '/ghome/group07/test/W6/task_1/x3d/weights/x3ds_valloss_pretrained.pth'
    ]
    for i, weigth_path in enumerate(weights_to_try):
        ...
        test_eval(model, loaders['testing'], loss_fn, args.device, None, description=f"Testing")
