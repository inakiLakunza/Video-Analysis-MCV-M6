...
def print_model_summary(
    ...
    if print_FLOPs:
        num_FLOPs = model_analysis.calculate_operations(model, clip_length, crop_size, crop_size, n_channels=n_channels, clip_length_lower=n_less_frames)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model', type=str, default='modified_resnet',
                        help='Used model, it can be "resnet" of "x3d"')
    ...
    wandb.init(
        project = "C6-W7-Task2",
        name = f"[{args.model}] no freeze 250 epochs, bs {args.batch_size} Optical Flow (Pretrained) & (All layers)",
        config={
            "epochs": args.epochs,
    ...
    print_model_summary(model, args.clip_length, args.crop_size, n_channels=5, n_less_frames=1)
    ...
    if args.model.lower() == 'x3d': 
        SAVE_NAME = "x3d_raft_resnet_pretrained_all_layers_no_freeze.pth"
    ...
    # EARLY STOPPING
    early_stopper = EarlyStopper(save_path, patience=30, min_delta=0.)
    #==========================================
