...
class TSNHMDB51Dataset(Dataset):
    ...
    def _create_transform(self) -> v2.Compose:
        ...
        if self.regime == TSNHMDB51Dataset.Regime.TRAINING:
            ...
            # return v2.Compose([
            #     v2.Resize((182*3, 182*3)), # Shortest side of the frame to be resized to the given size
            #     v2.CenterCrop((182*3, 182*3)),
            #     v2.ToDtype(torch.float32, scale=True),
            #     v2.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])
            # ])
            return v2.Compose([
    ...
    def __getitem__(self, idx: int) -> tuple:
        ...
        if video_len <= self.clip_length * self.temporal_stride:
            ...
            number_sequences = video_len // (self.clip_length * self.temporal_stride)

        splitted_video = np.array_split(frame_paths, self.n_segments)
        video = None

        for idx, chunk in enumerate(splitted_video):
            
            if number_sequences == 1:
                clip_begin = 0
                clip_end = len(chunk)
            else:
                clip_begin = random.randint(0, max(len(chunk) - self.clip_length * self.temporal_stride, 0))
                clip_end = clip_begin + self.clip_length * self.temporal_stride

            for ii, path in enumerate(chunk[clip_begin:clip_end:self.temporal_stride]):                  
                frame = read_image(path)  # (C, H, W)
                if video is None:
                    video = torch.zeros((self.n_segments, self.clip_length, 3, frame.shape[1], frame.shape[2]), dtype=torch.uint8)
        ...
        labels = torch.tensor(label).repeat(self.n_segments)
    ...
    def collate_fn(self, batch: list) -> dict:
        ...
        batched_labels = torch.cat([d for d in unbatched_labels], dim=0)

        return dict(
            clips=batched_clips, # (B, C, T, H, W)
            labels=batched_labels, # (K,)
