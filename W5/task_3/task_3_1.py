...
def evaluate_with_windows(clip, label, overlap=0):

    assert overlap < 4, "The chosen overlap must be lower than 4"
    ...
    for i in range(0, n_frames, jump):
        # get window of 4 elements
        if i+3 >= n_frames: break
        window = clip[:, :, i:i+4, :, :]
...
def evaluate(
    ...
    for batch in pbar:
        ...
        with torch.no_grad():
            ...
            sum_of_probs, avg_of_logits = evaluate_with_windows(clip, label, overlap=3)
...
if __name__ == "__main__":
    ...
    # we just need the models state dict and the validation loss it
    # used for saving the best model
    saved_val_loss = checkpoint['loss']
