...
def inference_on_dir(model,
    ...
    for test_id in range(0, len(filenames) - 1, stride):
        ...
        with open('../results/GMFlow_norefine.npy', 'wb') as f:
            print("Saving on ../results/GMFlow_norefine.npy ...")
            np.save(f, flow)

        output_file = os.path.join(output_path, os.path.basename(filenames[test_id])[:-4] + '_flow.png')
        output_file = '../results/flow_vis_GMFlow_norefine.png'
        # save vis flow
