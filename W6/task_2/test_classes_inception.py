...
def evaluate_no_temp_info(
    ...
    for batch in pbar:
        # Gather batch and move to device
        clips, labels = batch['clips'].to(device), batch['labels'].to(device)
        # (16, 3, 4, 299, 299) -> (16 * 4, 3, 299, 299)
        dims = clips.shape[2]
        clips = clips.view(-1, 3, 299, 299).to(device)
        labels = labels.repeat_interleave(dims).to(device)
        with torch.no_grad():
            outputs = model(clips)
            # Compute loss (just for logging, not used for backpropagation)
            loss = loss_fn(outputs, labels) 
            ...
            for i in range(len(labels)):
                acc_class_dict[labels[i].item()] = acc_class_dict.get(labels[i].item(), 0) + (float(hits_iter) / len(labels))
                class_cont[labels[i].item()] = class_cont.get(labels[i].item(), 0) + 1
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model-name', type=str, default='inception',
    ...
    # LOAD MODEL
    load_path = "/ghome/group07/test/W6/task_2/weights/best_val_model_epoch_40.pth"
