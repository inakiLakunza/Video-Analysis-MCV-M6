...
def create(model_name: str, load_pretrain: bool, num_classes: int) -> nn.Module:
    ...
    elif model_name == '3d_resnet_rgb':
        return create_3d_res_rgb(load_pretrain, num_classes)
    elif model_name == '3d_resnet_opf':
        return create_3d_res_opf(load_pretrain, num_classes)
    else:
        raise ValueError(f"Model {model_name} not supported")
    
def create_3d_res_opf(load_pretrain, num_classes):
...
def create_3d_res_rgb(load_pretrain, num_classes):
    model = torch.hub.load('facebookresearch/pytorchvideo', 'slow_r50', pretrained=True)
    model.blocks[-1].proj = nn.Identity()
    # freeze layers
